...
- **运行中终止与断点继续**：流程推进时输入框的发送键就地变「■ 终止」，点击在当前节点边界协作停下（LLM 生成近乎即时；真实测试执行等长节点等到节点收尾），断点落在最后一次节点完成的 checkpoint——输入框变「⏵ 继续」可从断点续跑，刷新页面后仍恢复该状态；终止后也可用「从此重来」回退换方向
- **CLI 孪生客户端**：同一套图与事件协议，`devflow setup / new / spec / resume / list / export / checklist / cases / check-providers / check-llm`
...
- **采纳 = 评审通过**：测试场景卡逐条勾选「采纳」，终审门禁弹出后点「✓ 提交评审」
  一键通过（`state.adopted_cases` 记录采纳集）；终审弹窗直接点「通过」等价——勾选了
  用例即走采纳链路，未勾选会先确认（直接通过则不采纳、用例不入库，通过后仍可
  「☰ 沉淀」归纳）。不做采纳则终审门禁照常通过/驳回。
  **采纳即登记**：被采纳的用例同时写入持久**用例库**（`data/case_library/<会话>.jsonl`，
  完整结构化字段 + `adopted_at` 溯源；`DEVFLOW_CASE_LIBRARY_ROOT` 可改库根），会话删除
  不丢失，跨会话可管理、可独立沉淀；通过后的 toast 可一键「☰ 用例库」直达查看。
...
- **用例库（跨会话采纳资产）**：TC-CHECKLIST 抽屉新增「用例库」页签——按来源浏览
  登记用例（关键词检索），单条删除 / 整来源注销 / 导入外部用例文件；勾选任意子集
  点「✦ 沉淀所选」即 AI 归纳为清单条目入库，**不经需求澄清 → 用例生成全流程**。
  清单路由门禁的空库 / 无匹配卡同样提供「☰ 从用例库沉淀清单」入口。
- **CLI**：`devflow checklist init`（生成模板 + payment 示例 + 内置通用清单：HTTP 接口 / 前端
  鉴权·布局·易用 / 数据库 SQL / Shell 脚本 / Skill 设计 / Agent 设计 / 自动化流水线 / 单元测试，
  共 172 条，`--no-general` 跳过）· `list`（库树）· `show <业务路径>` · `rm <业务路径>`（删除清单目录，
  删后可重新发布）· `distill --business <目录> [--ids/--thread/--keyword]`（独立沉淀：从用例库
  选用例归纳入库，`--yes` 跳过确认）。`devflow cases list / rm / import`：用例库管理——
  `list [--thread/--keyword]` 跨会话列用例、`rm` 删单条或 `--thread` 注销来源、`import`
  导入 `.csv`（CaseCraft 导出格式）/`.json`（用例数组或 test_report）登记入库（如 spec 自动
  模式导出的 `casecraft-tests-*.csv` 回灌）。CLI 终审 `approve` 后跟采纳清单（`all` /
  场景表序号 / TC 编号，如 `approve 1,3-5`）同样采纳即登记。
- **库浏览**：顶栏「☰ TC-CHECKLIST」（testcases 检查清单）从右侧展开浏览抽屉，分
  「清单库 / 用例库」两个页签——清单库页签提供标题导航 +
