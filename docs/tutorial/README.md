...
规则引擎按需求特征推荐图种类（流程图/时序图/状态图/ER 图/用户旅程图）并标注推荐理由，点卡片即选定。本需求含明显状态流转与超时分支，演示中选了推荐项。
...
| **CLI 脚手架** | 快速起步 | `python3 -m devflow.cli checklist init`（生成模板 + payment 示例 + **内置通用清单**，见附A.1）；`checklist list` / `show` 查看 |

`_` 开头的目录不参与路由。CLI 与 Web 共用同一份清单库。

### 附A.1 内置通用清单一览（checklist init 自带）

`checklist init` 默认播种 8 个开箱可路由的通用业务（共 172 条检查点，均按
「正向/反向/边界值/等价类/状态流转/场景法/安全/性能」八分节、P0–P2 优先级书写）。
跑需求时提到对应关键词，清单路由门禁会自动匹配预选；也可随时在 Web 顶栏
「☰ TC-CHECKLIST」抽屉里检索、浏览全文。

| 业务（rel_dir） | 条目 | 覆盖要点 | 触发关键词示例 |
|---|---|---|---|
| `api` · HTTP 接口 | 24 | 方法语义、状态码契约、参数校验、幂等与并发、注入/越权/HSTS/CORS、P95 性能 | 接口 / API / REST / 状态码 |
| `frontend` · 前端界面 | — | 业务壳（无自身清单），按子业务加载 | 前端 / 界面 / 页面 / 组件 |
| ↳ `frontend/auth` 安全鉴权 | 21 | 登录登出、防爆破、会话过期与改密失效、深链回跳、CSRF / 凭证存储 / 密码哈希 | 登录 / 鉴权 / 权限 / token |
| ↳ `frontend/layout` UI 布局 | 14 | 设计稿还原、分辨率与断点、空态错误态、浏览器与明暗主题兼容 | 布局 / 样式 / 响应式 / 兼容 |
| ↳ `frontend/usability` 用户易用 | 14 | 即时反馈、危险操作确认、表单纠错定位、草稿防丢、键盘可达 | 易用 / 交互 / 提示 / 引导 |
| `sql` · 数据库 SQL | 19 | DDL 完整性、禁止无条件变更、EXPLAIN 走索引、金额精度、迁移幂等可回滚 | 数据库 / 建表 / 迁移 / 索引 |
| `shell` · Shell 脚本 | 17 | set -euo pipefail、幂等重跑、退出码契约、mktemp+trap、不硬编码密钥 | 脚本 / bash / 命令行 |
| `skill` · Skill 设计 | 15 | 触发描述稳定命中、单一职责、渐进披露、组合衔接 | skill / 技能 / 提示词模板 |
| `agent` · Agent 设计 | 18 | 提示词契约、工具选择、幻觉落地校验、状态续跑、评测回归、注入防护 | agent / 智能体 / 工具调用 |
| `ci` · 自动化流水线 | 15 | 触发策略、密钥注入、幂等重跑、版本锁定、矩阵覆盖、回滚演练 | CI / 流水线 / 发布 / 工作流 |
| `unittest` · 单元测试 | 15 | 一测一行为、异常路径断言、mock 纪律、反 flaky（注入时钟/受控调度） | 单元测试 / 单测 / pytest / mock |

行为说明：

- **幂等不覆盖**：init 时已存在的业务一律跳过，用户对内置清单的修改不会被重置（payment
  示例除外——它是演示内容，重跑会重写）；
- **老库升级**：功能上线前初始化过的库根，重跑一次 `checklist init` 即可补齐缺失的内置业务；
- **只要空库模板**：`checklist init --no-example --no-general`；
- **内容源头**：内置清单定义在 `devflow/checklist/scaffold.py` 的 `BUILTIN_GENERAL`，增删改
  随仓库分发；
- **分工边界**：`shell` 管单个脚本质量、`ci` 管流水线编排；`unittest` 管代码级单测，端到端
  业务用例设计由沉淀出的各业务清单承担。
